...
object InterstitialTimeDelayAd {
    var interstitialAd: AdManagerInterstitialAd? = null
    private var isAdClicked = false
    private var tag = "InterstitialLogs"

    fun loadTimeDelayInterstitialAd(context: Context) {
        if (RemoteConstants.timeDelayInterstitialControl) {
            CommonData.debugLog(tag, "Load  InterstitialAd Method...")

            val adRequest = AdManagerAdRequest.Builder().build()

            AdManagerInterstitialAd.load(
                context, RemoteConstants.timeDelayInterstitialId, adRequest,
...
//    ) {
//        if (interstitialAd != null  && interstitialControl && !inAppPurchase) {
...
//    }
    ...
    fun showInterstitialAdWithDelay(
        ...
        val lastShown = SharedPreference.lastInterstitialShownTime
        val shouldShowAd =
            (currentTime - lastShown) >= (60_000L * RemoteConstants.interstitialShowDelay) || lastShown == 0L

        if (shouldShowAd && interstitialAd != null && RemoteConstants.timeDelayInterstitialControl && !RemoteConstants.inAppPurchase) {
            val ad = interstitialAd!!
            ad.show(activity)
            ad.fullScreenContentCallback = object : FullScreenContentCallback() {
                override fun onAdImpression() {
                ...
                override fun onAdShowedFullScreenContent() {
                    ...
                    AdController.isInterstitialShowing = true
                    isAdClicked = false
                    interstitialAd = null
                    SharedPreference.lastInterstitialShownTime = System.currentTimeMillis()
                }
                ...
                override fun onAdDismissedFullScreenContent() {
                    ...
                    AdController.isInterstitialShowing = false
                    loadTimeDelayInterstitialAd(activity)
                    listener?.onAdDismissed()
                }
            ...
            CommonData.debugLog(
                tag,
                "InterstitialAd not shown due to time delay or other conditions"
            )
            listener?.onAdDismissed()
        }
    }
